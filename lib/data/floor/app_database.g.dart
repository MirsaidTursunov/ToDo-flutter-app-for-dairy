    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `Tasks` (`time` TEXT NOT NULL, `dailyTaskText` TEXT NOT NULL, PRIMARY KEY (`time`))');
      ...
      : _queryAdapter = QueryAdapter(database, changeListener),
        _tasksInsertionAdapter = InsertionAdapter(
            ...
            (Tasks item) => <String, Object?>{
                  'time': item.time,
                  'dailyTaskText': item.dailyTaskText
                },
            changeListener),
  ...
  Stream<List<Tasks>> getBasketTasks() {
    return _queryAdapter.queryListStream('SELECT * FROM Tasks',
        mapper: (Map<String, Object?> row) => Tasks(
            ...
            dailyTaskText: row['dailyTaskText'] as String),
        queryableName: 'Tasks',
        isView: false);
  }
